...
def vsphere_host(resource, module_name):
    ...
    try:
        attrs.update({'ansible_ssh_host': raw_attrs['ip_address']})
    except (KeyError, ValueError):
        attrs.update({'ansible_ssh_host': ''})

    attrs.update({
        'consul_dc': _clean_dc(attrs['metadata'].get('consul_dc',
                                                     module_name)),
        'role': attrs['metadata'].get('role', 'none'),
    })
...
def main():
    ...
    default_root = os.environ.get('TERRAFORM_STATE_ROOT', os.path.abspath(
